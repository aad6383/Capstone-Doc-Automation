...
class CallbackBase(AnsiblePlugin):
    ...
    def run_playbook(self, playbook_path):
        # Here, we initialize the necessary objects for a running playbook
        loader = DataLoader()
        inventory = InventoryManager(loader=loader, inventory=inventory)
        variable_manager = VariableManager(loader=loader, inventory=inventory)
        
        # Set options for playbook execution
        options = {
            'listtags': False,
            'listtasks': False,
            'listhosts': False,
        ...
        # Contextual CLI arguments
        context_CLI = CLIArgs()
        for key, value in options.items():
            setattr(context_CLI, key, value)
    ...
    # Here, we will call the playbook.
    def v2_playbook_on_stats(self, stats):
        super(CallbackBase, self).v2_playbook_on_stats(stats)
        
        # Path to the playbook that sends the CSV to confluence
        playbook_path = "/home/dala0003/Capstone-Doc-Automation/playbooks/confluence.yml"
        self.run_playbook(playbook_path)
